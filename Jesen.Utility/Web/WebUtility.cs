...
namespace Jesen.Utility.Web
{
    public class WebUtility
    {
        ...
        /// <returns></returns>
        public static string WebParameterContract(Dictionary<string, object> parameterList)
        {
            StringBuilder sb = new StringBuilder();
            int index = 0;
            foreach (string parameter in parameterList.Keys)
            {
                if (index == 0) sb.Append("?" + parameter + "=" + parameterList[parameter]);
                else sb.Append("&" + parameter + "=" + parameterList[parameter]);
                index++;
            }
            return sb.ToString();
        }
